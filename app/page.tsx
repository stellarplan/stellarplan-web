...
export default function LandingPage() {
  return (
    <div className="min-h-screen text-foreground flex flex-col selection:bg-accent selection:text-white overflow-x-hidden" style={{ background: 'rgb(var(--background-rgb))' }}>
      ...
      <header className="sticky top-0 z-50 glass-panel border-0 border-b rounded-none">
        <div className="max-w-6xl mx-auto px-6 h-18 py-4 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-8 text-sm font-medium" style={{ color: 'hsl(var(--muted))' }}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center">
            ...
            <div className="lg:col-span-7 space-y-8 text-center lg:text-left">
              <motion.div
                ...
                className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider bg-accent-soft border border-accent-line text-accent-text"
              >
              ...
              <motion.h1
                ...
                style={{ fontSize: 'clamp(2.5rem, 5vw, 4rem)', fontWeight: 700, lineHeight: 1.1, letterSpacing: '-0.03em', fontFamily: 'var(--font-display)', color: 'hsl(var(--foreground))' }}
              >
                Your Money,{' '}
                <br className="hidden sm:inline" />
                <span className="text-accent-text">Protected Before You Spend It.</span>
              </motion.h1>
              ...
              <motion.div
                ...
                className="flex flex-wrap items-center justify-center lg:justify-start gap-6 text-xs font-medium text-muted"
              >
                <span className="flex items-center gap-2"><CheckCircle2 size={15} className="text-accent-text" /> Non-Custodial Vaults</span>
                <span className="flex items-center gap-2"><CheckCircle2 size={15} className="text-accent-text" /> Instant Salary Detection</span>
                <span className="flex items-center gap-2"><CheckCircle2 size={15} className="text-accent-text" /> Freighter Wallet Connect</span>
              </motion.div>
            </div>
            ...
            <div className="lg:col-span-5 relative">
              <motion.div
                ...
                className="relative rounded-3xl p-6 space-y-5 bg-surface border border-border"
              >
                {/* Header */}
                <div className="flex items-center justify-between pb-4 border-b border-border">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl grid place-items-center bg-accent-soft border border-accent-line text-accent-text">
                      <Wallet size={19} />
                    </div>
                    <div>
                      <p className="font-semibold text-sm text-foreground">Freighter Wallet</p>
                      <p className="text-xs font-mono text-muted">G...8F2A</p>
                    </div>
                  </div>
                  <span className="text-xs font-bold px-3 py-1 rounded-full bg-accent-soft text-accent-text border border-accent-line">Active</span>
                </div>

                {/* Balance */}
                <div className="rounded-2xl p-4 bg-surface-2 border border-border">
                  <p className="text-xs font-bold uppercase tracking-wider mb-1 text-muted">Salary Auto-Split Active</p>
                  <p className="text-3xl font-bold font-mono text-foreground">$2,500.00 <span className="text-sm text-muted-2">USDC</span></p>
                </div>
                ...
                {/* Footer */}
                <div className="flex items-center justify-center gap-2 py-2 rounded-xl text-xs font-medium bg-danger-soft border border-danger-line text-danger-text">
      ...
      {/* FEATURES */}
      <section id="features" className="py-24 relative bg-background border-t border-b border-border">
        <div className="max-w-6xl mx-auto px-6">
          <Reveal className="text-center max-w-2xl mx-auto mb-16 space-y-4">
            <h2 style={{ fontSize: '2.25rem', fontWeight: 700, letterSpacing: '-0.03em', color: 'hsl(var(--foreground))', fontFamily: 'var(--font-display)' }}>
              Built for Absolute Financial Discipline
            </h2>
            <p style={{ color: 'hsl(var(--muted))', lineHeight: 1.7 }}>
          ...
          <Stagger className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <StaggerItem
                key={title}
                className="vault-card p-8 space-y-5"
              >
                <div className="w-12 h-12 rounded-2xl grid place-items-center bg-accent-soft border border-accent-line text-accent-text">
                  <Icon size={22} />
                </div>
                <h3 style={{ fontSize: '1.1rem', fontWeight: 700, color: 'hsl(var(--foreground))', fontFamily: 'var(--font-display)' }}>{title}</h3>
                <p style={{ color: 'hsl(var(--muted))', fontSize: '0.875rem', lineHeight: 1.7 }}>{desc}</p>
              </StaggerItem>
            ))}
          </Stagger>
        </div>
      ...
      <section id="how-it-works" className="py-24 relative bg-background">
        <div className="max-w-6xl mx-auto px-6">
          <Reveal className="text-center max-w-2xl mx-auto mb-16 space-y-4">
            <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider bg-accent-soft border border-accent-line text-accent-text">
              How It Works
            </span>
            <h2 style={{ fontSize: '2.25rem', fontWeight: 700, letterSpacing: '-0.03em', color: 'hsl(var(--foreground))', fontFamily: 'var(--font-display)' }}>
          ...
          <Stagger className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <StaggerItem key={step} className="vault-card p-8 space-y-5">
                <span className="absolute top-6 right-6 font-mono text-sm font-bold text-muted-2">{step}</span>
                <div className="w-12 h-12 rounded-2xl grid place-items-center bg-accent-soft border border-accent-line text-accent-text">
                  <Icon size={22} />
                </div>
                <h3 style={{ fontSize: '1.1rem', fontWeight: 700, color: 'hsl(var(--foreground))', fontFamily: 'var(--font-display)' }}>{title}</h3>
      ...
      {/* SECURITY */}
      <section id="security" className="py-24 relative bg-background border-t border-border">
        <div className="max-w-6xl mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <Reveal className="space-y-6">
              ...
              <div className="flex items-center gap-3 rounded-xl px-4 py-3 bg-accent-soft border border-accent-line text-sm text-accent-text">
            ...
            <Stagger className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {[
                ...
                <StaggerItem key={title} className="vault-card p-6 space-y-3">
                  <div className="w-10 h-10 rounded-xl grid place-items-center bg-accent-soft border border-accent-line text-accent-text">
                    <Icon size={19} />
                  </div>
                  <h3 style={{ fontSize: '0.95rem', fontWeight: 700, color: 'hsl(var(--foreground))', fontFamily: 'var(--font-display)' }}>{title}</h3>
                  <p style={{ color: 'hsl(var(--muted))', fontSize: '0.8rem', lineHeight: 1.65 }}>{desc}</p>
                </StaggerItem>
              ))}
            </Stagger>
          </div>
      ...
      <section className="py-24 relative bg-background border-t border-border">
        <div className="max-w-4xl mx-auto px-6">
          <div className="relative overflow-hidden rounded-3xl p-10 md:p-14 text-center bg-surface border border-accent-line">
            <div className="relative z-10 space-y-6">
              <h2 style={{ fontSize: 'clamp(1.75rem, 4vw, 2.5rem)', fontWeight: 700, letterSpacing: '-0.03em', color: 'hsl(var(--foreground))', fontFamily: 'var(--font-display)', lineHeight: 1.15 }}>
                Stop rationing your paycheck by willpower.
              </h2>
              <p className="max-w-xl mx-auto" style={{ color: 'hsl(var(--muted))', lineHeight: 1.7 }}>
